...
export default {
  ...
  async mounted() {
    try {
      ...
      const blackhispanic = data.filter((crime) => crime.perp_race === 'BLACK HISPANIC')
      this.chartData.datasets[0].data.push(blackhispanic.length)
      ...
      const whitehispanic = data.filter((crime) => crime.perp_race === 'WHITE HISPANIC')
      this.chartData.datasets[0].data.push(whitehispanic.length)
